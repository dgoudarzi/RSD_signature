...
void expand_seed_binary_tree_bfs(sdith_full_key_t const* sk, seed_t root_seed,
  ...
  for (uint64_t d = 1; d <= PARAM_D; ++d) {
    #ifdef TEST_AES
    sdith_tree_aes_seed_expand(tree_ctx, current_level, previous_level, first_tweak, iteration, current_level_n, key_schedule_k0, key_schedule_k1);
    #endif
    #ifndef TEST_AES
...
void expand_seed_binary_tree_with_hint_bfs(seed_t *seed_hint, uint32_t hint,
  ...
  for (uint64_t d = 1; d <= PARAM_D; ++d) {
    ...
    #ifdef TEST_AES
    sdith_tree_aes_seed_expand(tree_ctx, current_level, previous_level, current_tweak, iteration, current_level_n, key_schedule_k0, key_schedule_k1);
    #endif
    #ifndef TEST_AES
